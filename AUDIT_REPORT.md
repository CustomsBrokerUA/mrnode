...
### 6.8 Адмінський бекфіл УКТЗЕД: `src/app/api/admin/backfill-hscodes/route.ts`

#### Сильні сторони
- Є авторизація через `getActiveCompanyFullAccess` + перевірка ролі `OWNER/MEMBER`.
- Є батчинг через `batchSize` + `cursor`.

#### Ризики (P1)
- **P1:** Endpoint сканує декларації з `xmlData != null`, тягне `xmlData` і парсить JSON у циклі. На великих даних це може бути важко.
- **P1:** Це “адмінський” endpoint, але він привʼязаний до активної компанії, не до глобального ADMIN. Варто перевірити, чи це очікувана модель.

#### Рекомендації
- **P1:** Додати “safety rails”:
  - ліміти на кількість викликів/хвилину
  - логування прогресу
  - можливість запуску як фонового job (не HTTP)

---

### 6.9 Синхронізація курсів валют: `src/app/api/exchange-rates/sync/route.ts`
...
- **Де:**
  ...
  - `src/app/api/admin/backfill-hscodes/route.ts`
